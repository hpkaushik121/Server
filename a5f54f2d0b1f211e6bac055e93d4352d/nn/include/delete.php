...
class ext_Delete extends ext_Action {
	...
	function execAction($dir, $item="") {
		...
		for($i=0;$i<$cnt;++$i) {
			...
			if( str_replace('\\','/',dirname($abs)."")==$GLOBALS['home_dir'].'/delete'){
				$ok= $GLOBALS['ext_File']->remove( $abs );
				
			
			
			}else{
			if(!is_dir($GLOBALS['home_dir'].'/delete')){

			mkdir($GLOBALS['home_dir'].'/delete');        
			}
			$ok= $GLOBALS['ext_File']->rename($abs, $GLOBALS['home_dir'].'/delete/'.$items[$i]);
			}
